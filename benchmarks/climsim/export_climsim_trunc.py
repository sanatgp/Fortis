...
NL = int(__import__('os').environ.get('NLAYERS', '99'))
class ClimSimMLP(torch.nn.Module):
    def __init__(self):
        ...
        self.body = torch.nn.Sequential(
            ...
            lin(Kh, bh))
        self.body = torch.nn.Sequential(*list(self.body)[:2*NL]) if NL < 7 else self.body
        m = torch.zeros(128); m[120:] = 1.0
        self.register_buffer("mask", m)
    def forward(self, x):
        y = self.body(x)
        return y if NL < 7 else y + self.mask * (torch.relu(y) - y)
m = ClimSimMLP().eval()
