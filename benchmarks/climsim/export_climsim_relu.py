...
K1 = W["dense_k"] / s[:, None]; b1 = W["dense_b"] - (mean / s) @ W["dense_k"]
...
class ClimSimMLP(torch.nn.Module):
    def __init__(self):
        ...
        self.body = torch.nn.Sequential(
            lin(K1, b1), torch.nn.ReLU(),
            lin(W["dense_1_k"], W["dense_1_b"]), torch.nn.ReLU(),
            lin(W["dense_2_k"], W["dense_2_b"]), torch.nn.ReLU(),
            lin(W["dense_3_k"], W["dense_3_b"]), torch.nn.ReLU(),
            lin(W["dense_4_k"], W["dense_4_b"]), torch.nn.ReLU(),
            lin(W["dense_5_k"], W["dense_5_b"]), torch.nn.ReLU(),
            lin(Kh, bh))
        m = torch.zeros(128); m[120:] = 0.0
        self.register_buffer("mask", m)
    def forward(self, x):
        y = self.body(x)
        return y + self.mask * (torch.relu(y) - y)   # relu on the last 8 outputs only
m = ClimSimMLP().eval()
cols = torch.tensor(W["columns"])
with torch.no_grad():
    ref = m(cols).numpy()
...
if len(sys.argv) > 1 and sys.argv[1] == "linalg":
    ...
    print(fx.export_and_import(m, cols[:1], output_type="linalg-on-tensors"))
else:
